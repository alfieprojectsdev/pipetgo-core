...
**Trigger:** invoke by saying "run the Compounding Protocol" at any milestone. Last run after PR #20 (2026-06-03).
...
- **Webhook state-transition writes must use `updateMany` with a guard predicate in `where` and check `count === 0` for concurrent-delivery early-return — never bare `update`.** A bare `update` cannot detect concurrent deliveries and silently overwrites a status already advanced by another request. `updateMany` with `{ id, guard_field: expected_value }` is the compare-and-set equivalent; `count === 0` means another delivery already advanced the state. This rule applies to every model whose status is advanced inside a webhook `$transaction` — not just `Payout`. See `src/features/payments/webhooks/handlers.ts` — `tx.transaction.updateMany({ where: { id: transaction.id, status: transaction.status } })` with `captureResult.count === 0` early-return — and `src/features/payments/payouts/handlers.ts` — `tx.payout.updateMany({ where: { id, externalPayoutId: null } })` with `updateResult.count === 0` early-return — as canonical examples.
...
- **Every `fetch()` call to an external payment provider must include `signal: AbortSignal.timeout(10_000)`.** An uncancelled fetch will hang a Server Action indefinitely if the provider is slow or unreachable; `AbortSignal.timeout` auto-cancels after 10 s and throws `TimeoutError`, which surfaces as an unhandled error rather than a silent hang. A catch block that only matches the provider's own error class (e.g., `XenditVaError`) will correctly rethrow the `TimeoutError` — do not add a generic fallback that would swallow it. See `src/lib/payments/xendit-va.ts` and `src/lib/payments/xendit.ts` — `signal: AbortSignal.timeout(10_000)` on every `fetch` call — as canonical examples.
- **A `"use client"` handler that `await`s a Server Action must surface every failure branch to a rendered error state — never let it return silently or reject unhandled — and any `window.open` must be called synchronously before the `await`, never after.** A handler that awaits a Server Action without handling its error branch drops a thrown referential-integrity error as an unhandled rejection and leaves a failed validation with no user feedback; a `window.open(url)` placed after the `await` is treated as non-user-initiated and silently blocked by popup blockers. Open the blank tab up-front, await the action, then on the success branch point the tab at the resolved URL and on the failure branch `win?.close()` + set an error state the UI renders. See `src/features/orders/spec-upload/ui.tsx` — `handleView` opens `window.open('', '_blank')` before the await, routes the `'url' in res` branch to the tab, and the error branch to `setViewError(...)` — as the canonical example.
- **Boundary inputs must be runtime-narrowed before use, and a value the schema guarantees must never carry a masking fallback.** `formData.get(x)` returns `FormDataEntryValue | null` (possibly a `File`) — narrow with `typeof x === 'string'` before calling string methods or passing to Prisma; never `as string`. A field the schema declares non-null (e.g. `User.email`) must not be `?? ''`-defaulted, and a status-coupled invariant (`kycStatus === 'REJECTED'` ⇒ a rejection reason exists) must `throw` when violated rather than `?? null`-ing it away — both silently substitute a wrong value for a contract violation that should surface in dev. See `src/features/admin/kyc-review/action.ts` — `typeof labIdValue === 'string' ? labIdValue : null` — and `src/features/labs/kyc-upload/page.tsx` — `if (lab.kycStatus === 'REJECTED' && !lab.kycRejectionReason) throw new Error(...)` — as canonical examples.
- **A `Date` already serialized to an ISO string at the RSC boundary must be rendered deterministically in client components — never via bare `new Date(iso).toLocaleString()`.** This is the client-side companion to the Decimal/Date serialization rule above: the RSC correctly hands the client an ISO string, then the client reintroduces nondeterminism. Argument-less `toLocaleString()` resolves locale and time zone from the runtime, so the server-rendered HTML differs from the client-rendered HTML — React reports a hydration mismatch and the timestamp visibly flips after hydration. Render with an explicit fixed locale **and** `timeZone` (e.g. `new Intl.DateTimeFormat('en-PH', { timeZone: 'Asia/Manila', dateStyle: 'medium', timeStyle: 'short' }).format(new Date(iso))`), or format the display string in the RSC and pass it down already-formatted. CodeRabbit flagged this 3× on PR #20. See `src/features/admin/order-oversight/ui.tsx` and `detail-ui.tsx` — `new Date(dto.createdAt).toLocaleString()` — as the anti-pattern to migrate (same debt repo-wide: `kyc-review/detail-ui.tsx`, `accreditation-review/detail-ui.tsx`).
