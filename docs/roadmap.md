...
| `Lab.isVerified` (ISO 17025) gate not yet enforced | **Low** — ITA 2023 solidary liability | ✅ T-18 merged (PR #18) — verify/reject UI + services/browse read gate + create-order write gate live; unverified labs cannot surface or receive orders. Remaining: `db push` audit columns per env, and an admin runbook for verifying ISO 17025 certs before first lab goes live. |
...
5/6 done (T-17 pulled into Phase 3, T-20 merged, T-15 done, T-13 KYC-review done, T-12 done).
...
**Phases 1–3 are complete.** T-13 KYC-review surface merged (closes the approve path for labs); T-18 accreditation (ISO 17025) merged (PR #18 — `Lab.isVerified` marketplace gate); T-12 (attachments) merged (PR #19 — SPECIFICATION + RESULT upload flows). The remaining T-13 scope is split into T-13b (read-only order oversight, pull forward only on real ops need) and T-13c (role management, deferred — privilege-escalation audit). T-19 (dispute/redress) remains blocked on schema migration.
