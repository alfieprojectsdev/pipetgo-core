 ...
 * documentType allowlist throws on unknown — never writes Lab.kycStatus. (ref: T-18 invariants)
 */
...
function makeFormData(overrides: Record<string, string> = {}): FormData {
  ...
  fd.append('fileName', overrides.fileName ?? 'cert.pdf')
  fd.append('mimeType', overrides.mimeType ?? 'application/pdf')
  fd.append('fileSize', overrides.fileSize ?? '1024')
  fd.append('documentType', overrides.documentType ?? 'ACCREDITATION_CERTIFICATE')
  return fd
}
...
describe('requestUploadUrl', () => {
  ...
  it('throws for documentType KYC_CERT — not in ACCREDITATION_CERTIFICATE allowlist', async () => {
    mockAuth.mockResolvedValue(LAB_ADMIN_SESSION)
    mocks.labFindUnique.mockResolvedValue(MOCK_LAB)

    await expect(
      requestUploadUrl(null, makeFormData({ documentType: 'KYC_CERT' })),
  ...
  it('happy path: labDocument.create uses documentType=ACCREDITATION_CERTIFICATE', async () => {
    mockAuth.mockResolvedValue(LAB_ADMIN_SESSION)
    mocks.labFindUnique.mockResolvedValue(MOCK_LAB)

    await requestUploadUrl(null, makeFormData())

    expect(mocks.labDocumentCreate).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ documentType: 'ACCREDITATION_CERTIFICATE' }),
      }),
    )
  })
