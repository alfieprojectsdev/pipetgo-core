...
| `handlers.ts` | `processSettlement` — three-layer idempotency (IdempotencyKey table + COMPLETED early-return + updateMany CAS); Payout QUEUED -> COMPLETED; LabWallet pendingBalance decrement + availableBalance increment in one $transaction; payout-hold predicate excludes DISPUTED orders from findFirst and updateMany (see README.md) | Modifying settlement logic, idempotency behavior, balance invariants, or the payout-hold predicate |
